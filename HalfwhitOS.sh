...
installleftwm && paru -S leftwm eww-git picom alacritty nerd-fonts pacwall-git hsetroot xsel xclip btop fish fisher starship topgrade fd exa ripgrep fetchit-git greetd greetd-tuigreeti rofi && systemctl --user enable pacwall-watch-packages.path && systemctl --user enable pacwall-watch-updates.timer && sudo systemctl enable greetd.service
